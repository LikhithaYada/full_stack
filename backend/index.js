...
const PORT = 6001;
mongoose.connect('mongodb://localhost:27017/SBLocal',{ useNewUrlParser: true, useUnifiedTopology: true }).then(()=>{
...
const PORT = 6001;
mongoose.connect('mongodb://localhost:27017/SBLocal', { useNewUrlParser: true, useUnifiedTopology: true })
  .then(() => {
    ...
    app.post('/add-location', upload.single('bannerImg'), async (req, res) => {
      try {
        const { title, description, city, newCity, cityBannerImg, address, otherThings, contributor, contributorId } = req.body;
    ...
    app.post('/upload-monument-img/:id', upload.single('newImg'), async (req, res) => {
      try {
